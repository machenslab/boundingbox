% original file name: scnPerturbation20200808_local.m
...
%   col.voltage = flipud(cbrewer('seq','Purples',ceil(1.5*par.networksize)));
  col.voltage = autumn(ceil(1.5*par.networksize));
  if par.delay > 0
%     col.readout = flipud(cbrewer('seq','Blues',ceil(1.5*par.dimension)));
    col.readout = winter(ceil(1.5*par.dimension));
  else
%     col.readout = flipud(cbrewer('seq','Greens',ceil(1.5*par.dimension)));
    col.readout = summer(ceil(1.5*par.dimension));
  end
