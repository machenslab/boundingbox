...
% the figure "Synaptic transmission delays cause uninformed spikes, 
...
for dimensionality = list.dimensionality
  for redundancy = list.redundancy
    for k = 1:trialnumber

      CDGM2021_SimulateDelayTrial(dimensionality,redundancy,delay,'full');
      CDGM2021_SimulateDelayTrial(dimensionality,redundancy,delay,'wide');
      CDGM2021_SimulateDelayTrial(dimensionality,redundancy,delay,'reduced');
...
function [par,his,hon,performance,networkerror,pro,option] = ...
  CDGM2021_SimulateDelayTrial(dimensionchoice,coveragechoice,perturbationchoice,networkchoice,varargin)
...
%       disp([num2str(iterationscompleted),' of ',num2str(iterationsintotal),' completed.'])
      disp(['Trial completed. (D=', ...
            ...
            num2str(par(setindex,repeatindex).coverage),', ',option.network,')'])
...
% %   hon_partial.target = hon.target;
  ...
  savefolder = [mfilefolder,filesep,'..',filesep,'newdata',filesep,option.network,filesep];
