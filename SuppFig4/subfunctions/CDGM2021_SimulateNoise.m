% script and functions to simulate a delayed network trials as in
...
% Includes data analysis to reduce each trial to a few key performance indicators.
% Includes optional code for plotting and formatting single-trial results.


for dimensionality = list.dimensionality
  for noisestd = list.noisestd
    for k = 1:trialnumber
      
      for redundancynarrow = list.redundancynarrow
          CDGM2021_SimulateNoiseTrial(dimensionality,redundancynarrow,noisestd,'narrownoise');
      end
      
      for redundancywide = list.redundancywide
          CDGM2021_SimulateNoiseTrial(dimensionality,redundancywide,noisestd,'widenoise');
      end
...
function [par,his,hon,performance,networkerror,pro,option] = ...
  CDGM2021_SimulateNoiseTrial(dimensionchoice,coveragechoice,perturbationchoice,networkchoice,varargin)
  ...
  elseif numel(varargin) == 2 && isempty(varargin{1})
...
%       disp([num2str(iterationscompleted),' of ',num2str(iterationsintotal),' completed.'])
      disp(['Trial completed. (D=', ...
            num2str(par(setindex,repeatindex).dimension),', M=', ...
            num2str(par(setindex,repeatindex).dimension),', sigma=', ...
            num2str(par(setindex,repeatindex).voltage.noisestd),', ',option.network,')'])
...
%   par.duration         = 10.4;
  ...
  [par,his,hon]   = analyseSCN(par,his,option);
...
%   inputtype = 'sine ND';
  switch inputtype
    case 'fixed'
%       par.input.value = ones(par.dimension, par.numtimestep);
%       his.input = par.input.value;
      his.input = -ones(par.dimension, par.numtimestep);
      his.input = 2*his.input; % FD20210510
%       his.input = 10*his.input; % FD20210509
    case 'frozen'
...
%         his.input = his.input * par.timestep;
    ...
    case 'smooth ramp interpolated' % as used for the paper
      ...
      if par.numinterval == 1
        ...
        % prepare smoothed signal noise
        timestepnuno = 1e-4;
        numtimestepnuno = round(par.numintervaltimestep * par.timestep/timestepnuno);
...
%     removepercentage = .2; % percentage of excitatory weights to be removed
  ...
  if option.disconnect  % completely disconnected neurons
    par.recurrent = zeros(size(par.recurrent));
  end
...
%   his.voltage(:,1) = randn(par.networksize,1); % FD20210505
...
function [par,his,hon] = analyseSCN(par,his,option)
  
  parallelcomputing = false;

  his.numspike = sum(his.spike,2);
...
%   % kernel = par.timestep * exp(-his.realtime/par.decoder.tau)/par.decoder.tau;
  ...
  if option.disconnect
    inputnormperstep  = sqrt(sum(his.input,1).^2);
    outputnormperstep = sqrt(sum(his.readout,1).^2);
%     scalefactor = mean(l2normperstep);
    scalefactor = max(inputnormperstep) / max(outputnormperstep);
    his.readout = his.readout * scalefactor;
  end
  
  % readout scaling:
  timeaveragednorm = mean(sqrt(sum(his.readout.^2,1)));
  scalingfactor = (timeaveragednorm + (par.polytopestretch-1)/2) / timeaveragednorm;
%   scalingfactor = (timeaveragednorm + (par.polytopestretch)) / timeaveragednorm;
